...
class StateGraphic extends GameObject
{
	 ...
	 */
	private static var texturesAnchor:Map<String,Array<Point>>;	
	...
	private static var texturesSize:Map<String,Array<Rectangle>>;	
	...
	static public function addTextures (pJson:Json): Void {
		...
		if (texturesAnchor == null) texturesAnchor = new Map<String,Array<Point>>();
		if (texturesCache == null) texturesCache = new Map<String,Array<Texture>>();		
		if (texturesSize == null) texturesSize = new Map<String,Array<Rectangle>>();		
		...
		for (lName in Reflect.fields(lFrames)) {
			...
			lNum = lNum == null ? 1 : lNum;
			if (texturesAnchor[lID] == null) texturesAnchor[lID] =  new Array<Point>();
			if (texturesAnchor[lID][lNum] == null) texturesAnchor[lID][lNum-1] = Reflect.field(lFrames, lName).pivot;
			if (texturesSize[lID] == null) texturesSize[lID] =  new Array<Rectangle>();
			if (texturesSize[lID][lNum] == null) {
				var lrect:Dynamic = Reflect.field(lFrames, lName).spriteSourceSize;
				texturesSize[lID][lNum - 1] = new Rectangle(lrect.x, lrect.y, lrect.w, lrect.h);
			}
		}
	}	
	...
	private function getAnchor (pState:String):Point {
		...
		var lFrames:UInt = texturesDefinition[lID];
		if (lFrames == 1) return texturesAnchor[lID][0];
		else return texturesAnchor[lID][anim.currentFrame];
		
		return texturesAnchor[lID][anim.currentFrame];
	}
	
	/**
	 * retourne la taille suivant le state
	 * @param	pState State de l'instance
	 * @return	l'ancre
	 */
	private function getTextureSize (pState:String):Rectangle {
		var lID:String;
		
		if (pState == DEFAULT_STATE) lID = assetName+ANIM_SUFFIX;
		else lID = assetName+"_" + pState+ANIM_SUFFIX;
		
		return texturesSize[lID][anim.currentFrame];
	}
